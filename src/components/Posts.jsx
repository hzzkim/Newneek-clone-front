  ...
  return (
    <div className="main-div flex flex-col max-w-screen-sm w-full mx-auto md:px-9 md:border-x md:border-x-gray-100">
      ...
      <div className="tabs-container mb-4">
        ...
        <button
          className={`tab ${currentFilter === "following" ? "active" : ""}`}
          onClick={() => handleTabClick("following")}
        >
          팔로잉
        </button>
        <button
          className={`tab ${currentFilter === "news" ? "active" : ""}`}
      ...
      {filteredPosts.map((post) => {
        ...
        // 최신 댓글 추출 및 정렬
        const latestComment = postComments.sort(
          (a, b) => new Date(b.date) - new Date(a.date)
        )[0];

        // 최신 댓글에 대한 대댓글 추출
        const replies = postComments.filter(
          (c) => c.parentId === latestComment?.commentId
        );
          ...
          <React.Fragment key={post.postId}>
            ...
            {latestComment && (
              <div className="commentContainer">
                <PostOrCommentItem
                  item={latestComment}
                  user={users.find((u) => u.id === latestComment.userId)}
                  postComments={[]}
                  replies={replies}
                />
...
const PostOrCommentItem = ({ item, user, postComments, replies }) => {
  ...
  const containerClass =
    isPost || replies.length > 0 ? "postContainer" : "commentContainer";
...
const Modal = ({ onClose, user }) => {
  ...
  const handleBlur = () => {
    if (contentRef.current.textContent.trim() === "") {
      contentRef.current.textContent = `${user.name} 님의 생각을 나누며 지식을 넓혀 보세요`;
    }
  };

  const submitPost = async () => {
    console.log("onclick이벤트 발생");
      ...
      <div className="modal">
        ...
        <div className="modal-content">
          ...
          <footer className="modalFooter">
            ...
            >
              <p>0 / 300</p>
              <button className="modal-submit" onClick={submitPost}>
                남기기
              </button>
            </div>
          </footer>
